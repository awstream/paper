...
or the reference results when all degradation operations are off.  We show
...
data, but only do so when we detect additional bandwidth. $(ii)$~Exhaustive
profiling is expensive. If it takes too much time, the newly-learned profile may
already be stale. \sysname{} uses a combination of parallelism and sampling
