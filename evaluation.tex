...
\begin{table}[t]
  ...
  \begin{tabular}{c c c}
    \toprule
    online scheme & overhead & savings \\
    \midrule
    continous & 52X & 1x \\
    partial data & 17X & 3x\\
...
\para{Sampling-based techniques speed up online profiling
  (\autoref{fig:online}).} Before we evaluate the speed up, we validate the
...
seconds), we save the online profiling by 3x (from 52X to 17X).
...
For our test data, this scheme is enough to correct concepts drifts by predicting an accurate bandwidth usage
