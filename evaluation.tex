...
\para{Large bandwidth variation.} For all three applications, there are three to
...
\para{Multiple dimensions achieve the optimal.} Comparing the dotted lines in
...
\para{Quantification with precision}. All the generated profiles are actionable
...
the profiling task by assigning configurations to machines (\fixme{or in the
  unit of GPU}). (i) In the offline profiling, because we do not assume a
...
\fixme{Explain experiment setup and why baselines are streaming over TCP/UDP?}
