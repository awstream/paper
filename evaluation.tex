...
bandwidth demand and application accuracy of one configuration. We highlight the
Pareto-optimal boundary $\mathbb{P}$ with blue dashed lines. To understand each
...
including streaming over TCP/UDP, JetStream and DASH. Our evaluation shows that
...
with JetStream that uses manual policies from the
paper~\cite{rabkin2014aggregation}. $(iii)$ The third baseline is from a
modified version of JetStream that can use the profile learned by \sysname{}. We
denote this baseline as JetStream++.\footnote{The appendix describes in details
  how we modified JetStream.}
...
The throughput figure in shows the effect of our traffic shaping. During the
shaping, TCP and UDP make full use of the available bandwidth; in comparison,
\sysname{}, JetStream, and JetStream++ are relatively conservative when
performing degradation. When we stop the shaping at t=440s, TCP has a
``catch-up'' phase when it's sending all queued items as fast as
possible. JetStream also has queued items because the manual policy (with
limited rules) cannot sustain 5 mpbs bandwidth. In comparison to all baselines,
...
leading to a consistent low latency.\footnote{FFmpeg discards packets that
  arrive later than a specific deadline (33ms for 30FPS).} Both JetStream and
JetStream++ are able to adapt during traffic shaping. But because JetStream's
runtime reacts instantaneously when the congestion condition changes, they end
up oscillating among polices. \sysname{} effectively addresses the oscillation
with probing and achieves a much lower latency than all baselines other than
UDP.
...
sends data at high fidelity, thus it has the highest accuracy of \textasciitilde
92\%. But this comes with the cost of high latency. JetStream uses manual
policies that are sub-optimal in comparison to our learned profile, so it
achieves a low accuracy of \textasciitilde 84\%. Using Pareto-optimal
...
achieves a high accuracy of \textasciitilde 89\% with a small variation.
...
delays. While the latency increases with the added delay, for all conditions,
\sysname{} mostly manages to achieve sub-second latency. We see a higher
variation in latency and more outliers as network delay increases. This is
caused by a longer time to detect congestion and adapt configurations. In terms
of accuracy, \sysname{} achieves similarly high accuracy.
...
mbps. Under this condition, PD runs with a higher accuracy \textasciitilde of
