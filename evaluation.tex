...
\para{Baselines.} $(i)$~The first baseline applications use vanilla TCP and
...
ffmpeg~\cite{bellard2012ffmpeg}. $(ii)$~The second baseline application is built
with JetStream~\cite{rabkin2014aggregation} that uses the manual policy in
\autoref{sec:making-case-sys-approach}. $(iii)$~The third baseline is from a
modified version of JetStream that can use the profile learned by \sysname{}. We
denote this baseline as JetStream++.\footnote{The appendix describes the details
  of our JetStream modifications.}
...
JetStream, and JetStream++ are relatively conservative with data degradation
(see their throughput drops). When we stop the shaping at t=440s, TCP has a
...
three rules) cannot sustain 5 mpbs bandwidth. In comparison to all baselines,
\sysname{} increases the throughput gradually because of the explicit probing
phase.
...
among polices. \sysname{} effectively addresses the oscillation with probing and
achieves a much lower latency than all baselines other than UDP.
...
\textasciitilde 89\% with a small variation.
...
the trade-off between fidelity and freshness.\footnote{The performance data for
  app-specific in \autoref{fig:intro} is obtained by feeding PD's profile to
  AR.}
