...
In this section, we evaluate the runtime performance by varying available
bandwidth across geo-distributed sites and compare \sysname{} with baselines
including streaming over TCP/UDP, JetStream. Our evaluation shows that
\sysname{} achieves low latency and high accuracy simultaneously across all
applications. Due to space constraints, we only present the result of AR. The
appendix includes evaluation results for PD and TK.

\para{Experiment setup.} We conduct our experiments on four geo-distributed
machines from Amazon EC2, spanning four different regions. Three act as worker
nodes and one acts as the aggregation server. The average RTTs from worker to
server are: 65.2 ms, 22.2 ms, 50.3 ms.
...
bandwidth. Our experiments involve four phases: (i) before t=200s, there is no
shaping; (ii) at t=200s, we limit the bandwidth to 7.5 mbps for 3 minutes ;
(iii) at t=380s, we further decrease the available bandwidth to 5 mbps; (iv) at
t=440s, we remove all traffic shaping. For UDP, HTB doesn't emulate the packet
loss or out-of-order delivery; so we use \texttt{netem} and configure the loss
probability according to the delivery rate. Because different pair-wise
connections have different capacity, we impose a \textit{background} bandwidth
limit---25 mbps---to simplify comparisons.

\para{Baselines.} $(i)$~Streaming over vanilla TCP and UDP. For streaming over
TCP, we re-use \sysname{}'s implementation but disable the adaptation. For
streaming video over UDP, we use RTSP/UDP with
ffmpeg~\cite{bellard2012ffmpeg}. $(ii)$~JetStream~\cite{rabkin2014aggregation}. We
use the manual policy described in
\autoref{sec:making-case-sys-approach}. $(iii)$~JetStream++. This is a modified
version of JetStream that can use the profile learned by
\sysname{}.\footnote{The appendix includes details about how we modified
  JetStream.} $(iv)$~HTTP Live Streaming (HLS). HLS represents the class of
HTTP-based adaptive video streaming techniques. We configured HLS with a chunk
size of 1 second (instead of typical 2-10 seconds) for low latency
streaming.\footnote{The appendix includes details about our HLS setup.}
...
JetStream, JetStream++, and HLS are relatively conservative because of data
degradation (see their throughput drops). When we stop the shaping at t=440s,
TCP has a ``catch-up'' phase when it's sending all queued items as fast as
...
throughput gradually because of the explicit probing phase. HLS is the most
conservative scheme; it recovers from degradation after 60 seconds.

The latency figures shows that \sysname{} is able to maintain sub-second
latency. During the traffic shaping, TCP queues items at the sender side for up
to hundreds of seconds. In contrast, UDP always transmits as fast as possible,
leading to a consistent low latency.\footnote{FFmpeg discards packets that
  misses a deadline (33ms for 30FPS).} Both JetStream and JetStream++ are able
to adapt during traffic shaping. But because JetStream's runtime reacts
instantaneously when the congestion condition changes, they end up oscillating
among polices. HLS's latency fluctuates around 2-3 seconds due to chunking,
buffering and network variations. \sysname{} effectively addresses the
oscillation with probing and achieves a much lower latency than all baselines
other than UDP.
...
The accuracy figures shows that other than UDP, most schemes are able to
...
92\%. But this comes with the cost of high latency. JetStream uses a manual
policy that are sub-optimal in comparison to our learned profile, so its
accuracy is low (\textasciitilde 84\%). Using Pareto-optimal configurations,
JetStream++ is able to achieve a higher accuracy (\textasciitilde 89\%); but
because JetStream runtime's policy oscillation, the accuracy has a large
variation. In contrast, \sysname{} chooses configurations carefully so that it
can stay in a steady state as much as possible.  It achieves a high accuracy of
\textasciitilde 89\% with a small variation. HLS adapts by mostly adjusting
resolution instead of frame rate, in this AR application, it also achieves high
accuracy because our analytical task is sensitive to frame changes. In our PD
application (figure shown in the appendix), HLS's resolution adaptation causes
accuracy drop to \textasciitilde 10\%.

In summary, \autoref{fig:ar-runtime} shows that \sysname{}'s adaptation achieves
low latency and high accuracy simultaneously. We present the performance during
traffic shaping in a different form in \autoref{fig:intro} to assist discussing
...
shaping. The one-way delays ranges from 50ms to 250ms, following a normal
distribution where the variation is 10\% of the additional delay, e.g.
$250\pm 25\text{ms}$. We add delays in both direction, so the additional RTT can
be as high as 500ms.
...
because \sysname{} stays in \texttt{Steady} state as much as possible and
accuracy only depends on the level of degradation, \sysname{} achieves similar
...
mbps. Under this condition, PD runs with a higher accuracy of
