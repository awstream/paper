...
% Furthermore, application-specific optimizations do not generalize. A
% fine-tuned adaptation algorithm for one application will work poorly for
% another application, if performance metrics or data distributions change.  For
% example, video streaming focuses on quality of experience
% (QoE)~\cite{michalos2012dynamic, pantos2016http, yin2015control}. Because
% humans favor smoothness over image quality, video streaming systems maintain a
% high frame rate, e.g.\,\(25~\text{FPS}\), and reduce image resolutions under
